...
from mspray.libs.ona import fetch_osm_xml_data
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for location in locations:
            ...
            for submission in queryset:
                ...
                if submission_osm:
                    ...
                    for way in submission_osm_obj.findall("way"):
                        district = submission.location.parent.parent.name
                        health_facility = submission.location.parent.name
            ...
            if osm is not None:
                osm_file = open(
                    "/home/ukanga/data/mspray/2016/osm/%s.osm.xml"
                    % location.name,
                    "wb",
                )
                osm_file.write(
                    etree.tostring(osm, encoding="utf-8", xml_declaration=True)
                )
                osm_file.close()
