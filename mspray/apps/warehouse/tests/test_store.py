...
class TestStore(TestBase):
    ...
    def test_get_druid_intervals_use_timestamp_false(self):
        ...
        self.assertEqual(
            expected, get_druid_intervals(queryset, use_timestamp=False)
        )

    def test_get_druid_intervals_use_timestamp_true(self):
        """ test that we get the right intervals back """
        queryset = SprayDay.objects.all().order_by(
            OrderBy(RawSQL("LOWER(data->>%s)", ("end",)), descending=False)
        )
        first = queryset.first().data.get("end", queryset.first().spray_date)
        last = queryset.last().data.get("end", queryset.last().spray_date)
        expected = "{}/{}".format(first, last)
        self.assertEqual(
            expected, get_druid_intervals(queryset, use_timestamp=True)
        )
    ...
    def test_get_s3_url_hadoop_true(self):
        ...
        index tasks
        """
        if not hasattr(settings, "AWS_S3_HADOOP_BASE_URL"):
            self.skipTest("AWS_S3_HADOOP_BASE_URL is not configured.")
    ...
    def test_get_historical_data_day(self, json_file_mock, ingest_mock):
        ...
        filename = (
            "{}/".format(settings.DRUID_SPRAYDAY_DATASOURCE)
            + path
            + "/sprayday.json"
        )
    ...
    def test_get_historical_data(
        self, json_file_mock, ingest_mock, intervals_mock
    ):
        """
